...
async function monitor(root, meta, scannedProject, options, pluginMeta, targetFileRelativePath, contributors) {
    ...
    if (scannedProject.depGraph) {
        return await monitorDepGraph(root, meta, scannedProject, pluginMeta, targetFileRelativePath, contributors);
    }
    ...
    if (package_managers_1.GRAPH_SUPPORTED_PACKAGE_MANAGERS.includes(packageManager)) {
        ...
        if (monitorGraphSupportedRes.ok) {
            return await experimentalMonitorDepGraphFromDepTree(root, meta, scannedProject, pluginMeta, targetFileRelativePath, contributors);
        }
        if (monitorGraphSupportedRes.userMessage) {
            debug(monitorGraphSupportedRes.userMessage);
        }
    }
    return await monitorDepTree(root, meta, scannedProject, pluginMeta, targetFileRelativePath, contributors);
}
exports.monitor = monitor;
async function monitorDepTree(root, meta, scannedProject, pluginMeta, targetFileRelativePath, contributors) {
    let treeMissingDeps = [];
    const packageManager = meta.packageManager;
    ...
    const policyLocations = [policyPath]
        .concat(policy_1.pluckPolicies(depTree))
        .filter(Boolean);
...
async function monitorDepGraph(root, meta, scannedProject, pluginMeta, targetFileRelativePath, contributors) {
    const packageManager = meta.packageManager;
    ...
    const policyPath = meta['policy-path'] || root;
    const policyLocations = [policyPath]
        .concat(policy_1.pluckPolicies(depGraph))
        .filter(Boolean);
 ...
 */
async function experimentalMonitorDepGraphFromDepTree(root, meta, scannedProject, pluginMeta, targetFileRelativePath, contributors) {
    const packageManager = meta.packageManager;
